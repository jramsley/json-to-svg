...
function interpolateBSpline({ controlPoints, weights, knots, degree } = {}) {
  const polyline = [];
  ...
  const interpolationsPerSplineSegment = 25;
...
class Spline extends BaseEntity {
  constructor(config = {}) {
    super(config);
    this.controlPoints = config.data.controlPoints.map(controlPoint => { return { x: controlPoint[0], y: controlPoint[1] }; });
    this.weights = config.data.weights;
    this.knots = config.data.knots;
    this.degree = config.data.degree;

    this.vertices = interpolateBSpline({
      controlPoints: this.controlPoints,
      weights: this.weights.length ? this.weights : undefined,
      knots: this.knots,
      degree: this.degree
    }).map(value => {
      return {
